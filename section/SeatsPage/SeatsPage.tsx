...
export default function SeatsPage() {
  ...
  useEffect(() => {
    ...
    const handleChange = (payload: RealtimeChangePayload) => {
      ...
      // Ignore events we triggered ourselves so the screen doesn't jitter
      ...
      }, 500) // Lowered to 500ms for snappier realtime updates
    }
  ...
  useEffect(() => {
    ...
    return () => {
      // Only run this if we have actually locked seats in the database
      if (lastSyncedRef.current.length > 0 && selectedShowtime?.id) {

        // We CANNOT use `await` inside a synchronous cleanup function, 
        // but we CAN fire off the mutation to run in the background.
        console.log("User left SeatsPage. Releasing seats in background...");
        releaseSeatsAction({
          showtimeId: selectedShowtime.id,
  ...
  }, [selectedShowtime?.id, releaseSeatsAction]);
